module "topics_get_api" {
  ...
  request_models       = {}
  response_models      = {
                           "application/json" = aws_api_gateway_model.topics_get_response_model.name
                         }
}

resource "aws_api_gateway_model" "topics_get_response_model" {
  rest_api_id  = var.api_id
  name         = "topicsgetresponse"
  description  = "A JSON schema for GET /topics response"
  content_type = "application/json"
  schema       = file("${path.module}/response_schemas/topics_get.json")
}
